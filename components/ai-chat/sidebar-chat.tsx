...
export function SidebarChat({ onNewChatRef }: SidebarChatProps) {
  ...
  // Auto-scroll when streaming starts (after 1 second)
  useEffect(() => {
    // Detect when streaming starts (new streaming id appears)
    if (streamingMessageId && streamingMessageId !== prevStreamingIdRef.current) {
  ...
  const handleSubmit = useCallback(
    async (event?: React.FormEvent<HTMLFormElement>) => {
      ...
      requestAnimationFrame(() => {
        conversationRef.current?.scrollToBottom({ behavior: "instant" });
      });
    },
