...
import type { ChatToolDefinition, EnabledTools, ToolProvider } from "@/lib/ai/tools";
...
export function ToolItem({ tool, enabledTools, onToggleProvider }: ToolItemProps) {
  ...
  const inline = tool.toggle === "inline";
  const inlineProvider = inline ? tool.providers[0] : null;
  ...
  return (
    <div className="flex items-center gap-3 px-2 py-2">
      ...
      {inline && inlineProvider ? (
        <Switch
          checked={isProviderEnabled(enabledTools, tool.id, inlineProvider.id)}
          onCheckedChange={(checked: boolean) =>
            onToggleProvider(tool.id, inlineProvider.id, checked)
          }
        ...
        <Popover>
          ...
          >
            <div className="max-h-[200px] overflow-auto">
              {tool.providers.map((provider: ToolProvider) => {
