...
| 2026-04-19 | fix | Lifted the `useWorkspaceSettingsSync` bootstrap guard from `useRef(false)` to **module-scope** `let bootstrapped = false`. `app/files/[[...path]]/layout.tsx` wraps children in `<Suspense>` + `await connection()`, which remounts the entire workspace tree on every file switch — resetting the component-scoped ref and re-firing the bootstrap effect with the stale server value, which silently re-disabled the header centered-layout toggle. Module scope survives the remount; full page reloads still re-bootstrap as intended. |
