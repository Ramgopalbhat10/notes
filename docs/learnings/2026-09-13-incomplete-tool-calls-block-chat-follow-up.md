...
- Fix: Mark interrupted tool parts as `output-error` on the client (including the outgoing request body); sanitize + `ignoreIncompleteToolCalls` on the server. `createExtractTool` requests full JS-rendered content, live-fetches time out at 25s, and execute is hard-capped at 35s.
